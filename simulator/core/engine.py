...
class DESEngine:
    ...
    def _handle_block_received(self, event: Event) -> None:
        ...
        # 4. CPU core availability (handled by SimPy Resource)
        ...
        self.state.schedule_event(
            time_ms=self.state.current_time_ms + verify_time,
    ...
    def _create_block(self, proposer: Node) -> Block:
        ...
        max_txs = min(max_txs, 10000)  # Cap for simulation performance
    ...
    def _compute_results(self) -> SimulationResult:
        ...
        # Compute stale rate
        # A block is "stale" if propagation p90 exceeds half the block time
        stale_threshold = self.block_time_ms * 0.5
