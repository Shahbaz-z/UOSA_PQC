...
class TestVerificationProfiles:
    ...
    def test_slh_dsa_slow_variants(self):
        ...
        Per Cloudflare PQC benchmarks (2024): 'f' optimises for signing speed
        at the cost of larger signatures and slower verification.  The 's'
        variants produce smaller signatures with faster verify.
        """
        assert VERIFICATION_PROFILES["SLH-DSA-128f"].verify_time_us > VERIFICATION_PROFILES["SLH-DSA-128s"].verify_time_us
        assert VERIFICATION_PROFILES["SLH-DSA-256f"].verify_time_us > VERIFICATION_PROFILES["SLH-DSA-256s"].verify_time_us
