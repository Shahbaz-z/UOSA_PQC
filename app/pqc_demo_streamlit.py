...
Three tabs:
1. KEM Demo – Kyber key encapsulation interactive walkthrough
2. Signature Demo – Dilithium / Ed25519 / Hybrid signing
3. Block-Space Visualizer – Solana throughput impact analysis
"""
...
from blockchain.solana_model import compare_all, SIGNATURE_SIZES, SOLANA_BLOCK_SIZE_BYTES, SOLANA_BASE_TX_OVERHEAD
...
# Tab layout
# ---------------------------------------------------------------------------
tab_kem, tab_sig, tab_block = st.tabs([
    "KEM Demo",
    "Signature Demo",
...
# ===== TAB 1: KEM Demo ====================================================
with tab_kem:
    st.header("Kyber Key Encapsulation Mechanism")
    st.markdown(
...
with tab_sig:
    st.header("Digital Signatures: Dilithium, Ed25519 & Hybrids")
    st.markdown(
        "Dilithium is a lattice-based signature scheme from the NIST PQC standards. "
        "Hybrid mode concatenates an Ed25519 signature with a Dilithium signature, "
...
with tab_block:
    st.header("Solana Block-Space Impact Analysis")
    st.markdown(
        "This model estimates how replacing Ed25519 signatures with post-quantum "
        "alternatives would affect Solana transaction throughput. "
        "\n\n**Limitations:** This models *signature contribution* to transaction "
        "size only. Real transactions include additional overhead from account "
        "addresses, instructions, and blockhash."
    )
    ...
    comp = compare_all(block_size, base_overhead, slot_time)
        ...
        summary_df = pd.DataFrame([
            {
                ...
                "vs Ed25519": f"{a.relative_to_ed25519:.1%}",
    ...
    with chart_col1:
        st.plotly_chart(block_space_chart(comp.analyses), use_container_width=True)
    with chart_col2:
        st.plotly_chart(throughput_comparison_chart(comp.analyses), use_container_width=True)

    st.plotly_chart(signature_size_comparison(comp.analyses), use_container_width=True)
    ...
    st.markdown(f"""
- **Ed25519 baseline**: {baseline.txs_per_block:,} txs/block ({baseline.throughput_tps:,.0f} TPS)
- **Worst case ({worst.signature_type})**: {worst.txs_per_block:,} txs/block ({worst.throughput_tps:,.0f} TPS) — **{worst.relative_to_ed25519:.1%}** of baseline
- Dilithium signatures are **{SIGNATURE_SIZES['Dilithium3'] // SIGNATURE_SIZES['Ed25519']}x** larger than Ed25519
- Hybrid signatures (Ed25519 + Dilithium) provide quantum resistance at the cost of ~{100 - round(comp.analyses[-1].relative_to_ed25519 * 100)}% throughput reduction
""")
