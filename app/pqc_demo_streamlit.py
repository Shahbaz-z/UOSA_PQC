...
from blockchain.zk_models import (
    ZK_PROOF_SYSTEMS, ZKProofParams,
    compare_all_zk_proofs, build_zk_vs_signatures_table,
    SNARK_SYSTEMS, STARK_SYSTEMS, QR_PROOF_SYSTEMS,
...
with tab_zk:
    ...
    for a in zk_analyses:
        params = ZK_PROOF_SYSTEMS[a.proof_system]
        zk_rows.append({
    ...
    with zm1:
        st.metric(
            "Best Quantum-Resistant",
            f"{best_qr.proof_system}",
        )
        st.caption(
    ...
    with zk_ch1:
        st.plotly_chart(
            zk_proof_size_vs_gas_chart(zk_analyses),
            use_container_width=True,
        )
    with zk_ch2:
        st.plotly_chart(
            zk_throughput_comparison_chart(zk_analyses),
            use_container_width=True,
        )

    st.plotly_chart(
        zk_gas_breakdown_chart(zk_analyses),
        use_container_width=True,
    )
    ...
    st.dataframe(
        pd.DataFrame(zk_vs_sig_rows),
        use_container_width=True,
        hide_index=True,
    )

    st.plotly_chart(
        zk_vs_signatures_chart(zk_vs_sig_rows),
        use_container_width=True,
    )
